...
function renderCardDetail(i){if(!appData.cards[i])return;const c=appData.cards[i];const s=calcCard(c);const p=c.limit>0?(s.debt/c.limit)*100:0;document.getElementById('card-name').innerText=c.name;document.getElementById('card-limit').innerText=`Lim: ${fmt(c.limit)}`;document.getElementById('credit-balance-alert').classList.toggle('d-none',!(c.creditBalance>0));document.getElementById('credit-balance-amount').innerText=fmt(c.creditBalance);document.getElementById('card-used').innerText=fmt(s.debt);document.getElementById('card-avail').innerText=fmt(s.avail);
const cm = document.getElementById('card-monthly'); if(cm) cm.innerText=fmt(s.monthly);
document.getElementById('card-percent-text').innerText=`${p.toFixed(1)}%`;document.getElementById('card-progress').style.width=`${p}%`;const h=document.getElementById('card-visual-bg');const n=c.name.toLowerCase();let bg='linear-gradient(135deg, #2c3e50 0%, #4ca1af 100%)';if(n.includes('nu'))bg='linear-gradient(135deg, #82269e 0%, #a450c0 100%)';else if(n.includes('bbva'))bg='linear-gradient(135deg, #004481 0%, #2dcccd 100%)';else if(n.includes('santander'))bg='linear-gradient(135deg, #ec0000 0%, #ff4b4b 100%)';else if(n.includes('mercado'))bg='linear-gradient(135deg, #009ee3 0%, #00c6fb 100%)';else if(n.includes('stori'))bg='linear-gradient(135deg, #00a5a3 0%, #35dcb4 100%)';else if(n.includes('didi'))bg='linear-gradient(135deg, #ff7e00 0%, #ffac4d 100%)';h.style.background=bg;const t=document.getElementById('transactions-body');t.innerHTML='';if(c.transactions.length===0){t.innerHTML=`<tr><td colspan="5" class="text-center text-muted py-4">Sin movimientos</td></tr>`}else{c.transactions.forEach((x,ix)=>{let pd=(x.amount/(x.months||1))*(x.paidCycles||0);let r=x.amount-pd;if(r<0)r=0;t.innerHTML+=`<tr><td><div class="fw-bold text-dark">${x.desc}</div><div class="small text-muted">${x.paidCycles}/${x.months} pagos</div></td><td><span class="badge bg-light text-dark border">${x.months} M</span></td><td class="text-end text-muted small">${fmt(x.amount)}</td><td class="text-end fw-bold text-dark">${fmt(r)}</td><td class="text-end"><button class="btn-icon btn-icon-del shadow-sm" onclick="delTransaction(${ix})"><i class="fas fa-trash text-danger" style="font-size:0.8rem"></i></button></td></tr>`})}}
function updateChart(){const c=document.getElementById('mainChart').getContext('2d');if(myChart)myChart.destroy();
...
function savePurchase(){const i=document.getElementById('card-selector').value;const d=document.getElementById('new-purch-desc').value;const a=parseFloat(document.getElementById('new-purch-amount').value);const m=parseInt(document.getElementById('new-purch-months').value);if(d&&a>0){appData.cards[i].transactions.push({desc:d,amount:a,months:m,paidCycles:0});saveData();updateUI();bootstrap.Modal.getInstance(document.getElementById('addPurchaseModal')).hide()}else{alert("Completa datos")}}
